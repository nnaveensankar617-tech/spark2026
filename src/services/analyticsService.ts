...
import { getTopKey, normalizeAnalyticsEvent } from './analyticsUtils';
...
export class AnalyticsService {
  ...
  track(event: AnalyticsEvent): void {
    ...
    if (normalizedEvent.eventId) {
      this.updateEventAnalytics(normalizedEvent);
    }
    
    this.updateUserAnalytics(normalizedEvent);
  }
  ...
  /**
   * Get a concise analytics summary for a specific event
   */
  getEventSummary(eventId: string): {
    ...
    const analytics = this.getEventAnalytics(eventId);
    const peakHourKey = getTopKey(analytics.popularTimes);
    ...
    return {
      ...
      peakHour: peakHourKey ? Number(peakHourKey) : undefined,
      topDeviceType: getTopKey(analytics.deviceTypes),
      topReferralSource: getTopKey(analytics.referralSources),
    };
  }
